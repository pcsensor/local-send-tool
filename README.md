...
- **高安全性保障**：
  - **控制面仅限本机**：Web 控制类接口（`/api/runtime`、`/api/config` 读写、`/api/web/message`、`/api/web/file`）只接受来自 localhost 的请求，局域网其他设备无法远程改写本机配置或驱使本节点代发文件。接收类接口（`/api/message`、`/api/file/*`）仍对局域网开放以便正常收件。
